...
#' @title Extracting model results and performing FDR correction
#'
#' @description Function for extracting p-values and coefficients from \code{RunModel} output.
#' Also performs FDR correction, either over all peptides/proteins or
#' over all peptides of each protein individually
#'
#' @usage SummarizeModelResults(resModel, evalCovariable = "Condition_OLS",
#' correctPval = "all", pAdjust = "fdr",
#' annotPP = NULL, infoFeature = "Peptide", infoProtein = "Protein")
#'
#' @param resModel a list object, output from runModel function.
#' \\code{AnalyzeLiPPepData}\\code{AnalyzeTrpPepData}\\code{AnalyzeTrpProtData}.
#' @param evalCovariable character string giving name of columns in which coefficients and
#' P values of the variable of interest can be found.
#' Default is set to 'Condition_OLS'.
#' @param correctPval a character string defining how p-values should be adjusted.
#' Per default all p-values are corrected together, and it is set to 'all'.
#' Alternatively, FDR can be performed protein-wise, please set the variable to
#''protein-wise' for this.
#' @param pAdjust a character string giving the correction method to use when performing FDR
#' Check ?p.adjust for more information.
#' Set to 'fdr' (Benjamini & Hochberg (1995)) by default.
...
SummarizeModelResults <- function(resModel, evalCovariable = "Condition_OLS", correctPval = "all",
    ...
    modelSum <- data.frame(Coefficient = modelCoef[order(modelPvalAdj, decreasing = F)],
                           Pvalue = modelPval[order(modelPvalAdj, decreasing = F)],
                           Padj = modelPvalAdj[order(modelPvalAdj, decreasing = F)])
    return(modelSum)
}

#' @title Performing protein-wise FDR
#' @param vecPval a named numeric vector with P values and corresponding features
#' @param pAdjust a character string giving the correction method to use in \code{p.adjust},
#' Check \code{p.adjust} for more information.
#' Set to 'fdr' (Benjamini & Hochberg (1995)) by default.
#' @param annotPP a data.frame with peptide and protein annotation.
#' Rows are features and columns must include \code{infoFeature} and \code{infoProtein}.
#' Is only required if \code{correctPval} is set to 'protein-wise'.
...
#' @param infoProtein a character string providing column name of \code{annotPP} where
#' protein names/groups are noted. If \code{correctPval} is defines as 'protein-wise'
#' peptides with the same \code{infoProtein} valueWDD will be corrected together
#' Default is set to 'Protein'.
#'
...
#' #' sigOnly = T, pvalCut = 0.1, savePlots = F, filePath = NULL)
#' #'
...
#' #' @keywords internal
#' #'
#' CreatePepProtPlot <- function(plotData, Yaxis = "Yaxis", start = "start", end = "end", color = "Pval", nameY = "Coefficient", showPv = F){
#'     allProt <- names(plotData)
#'     orderSig <- unlist(lapply(plotData), function(x){
#'         min(x[, "Pval"])
#'     })
#'     plotList <- plotList[order(orderSig, decreasing = F)] # order proteins so that proteins with most significant peptides are plotted first
...
#'         ggpubr::ggarrange(plotList[[1]], plotList[[2]], plotList[[3]], nrow = 3, common.legend = T, legend = "right")
#'     })
#'     grDevices::dev.off()
#' }
#'
