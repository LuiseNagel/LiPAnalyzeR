...
#' lowBVLS = c(-1e9, 0, 0), upBVLS = c(Inf, Inf, Inf), addBVLSbounds = F,
#' LiPonly = F)
#'
...
#' Names of the list items should be set to "LiPPep", "TrpPep" and "TrpProt"
#' (if the LiP only version is run, names should refer to "LiPPep" and "LiPProt")
...
#' @param lowBVLS a numeric vector defining lower boundaries of the coefficients of the BVLS models.
#' Elements refer to definition of \code{formulaBVLS}.
#' Default is defined as 'c(-Inf, 0, 0)'.
#'@param upBVLS a numeric vector defining upper boundaries of the coefficients of the BVLS models.
#' Elements refer to definition of \code{formulaBVLS}.
#' Default is defined as 'c(Inf, Inf, Inf)'.
#' @param addBVLSbounds a boolean value, if TRUE as many bounds as additionally needed
#' in each BVLS model are added to \code{lowBVLS} and \code{upBVLS}.
#' Added boundaries are automatically set to -Inf for \code{lowBVLS} and Inf for \code{upBVLS}.
...
AnalyzeLiPPepData <- function(spectroList, annotS, infoCondition = "Condition",
    ...
    LiPOut <- RunModel(spectroList = spectroList,
                       ...
                       LiPonly = LiPonly)
    #colnames(LiPOut$modelCoeff)[2:3] <- c("TrpPep_BVLS", "TrpProt_BVLS")
    return(LiPOut)
}

#' @title Fitting model for PK-indendepent changes in Trp peptides

#' @description Function to build linear regression models for fitting MS data
#'
#' @usage AnalyzeTrpPepData(spectroList, annotS, infoCondition = "Condition",
#' formulaBVLS = "Y~XProt", formulaOLS = NULL,
#' lowBVLS = c(-1e9, 0), upBVLS = c(Inf, Inf), addBVLSbounds = F)
#'
#' @param spectroList a list of matrices, containing peptide/protein quantities.
#' Rows represent features and columns refer to the samples.
#' Names of the list items should be set to "LiPPep", "TrpPep" and "TrpProt"
...
#' infoCondition = "Condition", infoProtName = "Protein",
#' formulaBVLS = NULL, formulaOLS = NULL,
#' lowBVLS = NULL, upBVLS = NULL, addBVLSbounds = F,
#' LiPonly = F)
#'
#' @param spectroList a list of matrices, containing peptide/protein quantities.
#' Rows represent features and columns refer to the samples.
...
#' @param annotS a data.frame object containing sample annotation.
#' Rows are samples and must match to columns of \code{spectroList}.
#' @param annotPP data.frame with peptide and protein annotatioon.
#' Rows are features and must match to the row.names of SpectroList/QuantityMatrix.
#' @param infoCondition a character, providing column name of \code{annotS} in which condition is provided.
#' Default is 'Condition'.
#' @param infoProtName a character, providing column name of \code{annotPP} in which the protein names are provided.
#' Default is 'Protein'.
#' @param formulaBVLS a character string or formula defining the BVLS models.
...
AnalyzeTrpProtData <- function(spectroList, annotS, annotPP,
    ...
    Peps2Prots <- split(row.names(protData), annotPP[row.names(protData), infoProtName])
...
#' @description Function to build linear regression models for fitting MS data
#'
#' @usage RunModel(spectroList, annotS = NULL, formulaBVLS = "Y~XPep+XProt", formulaOLS = "Y~Condition",
#' lowBVLS = c(-1e9, 0, 0), upBVLS = c(Inf, Inf, Inf), addBVLSbounds = F,
#' returnBVLSmodels = F, returnOLSmodels = F, LiPonly = F)
#'
#' @param spectroList a list of matrices, containing peptide/protein quantities.
#' Rows represent features and columns refer to the samples.
#' Names of the list items should be set to "LiPPep", "TrpPep" and "TrpProt"
...
#' @param annotS a data.frame object containing sample annotation.
#' Rows are samples and must match to columns of \code{spectroList}.
#' @param formulaBVLS a character string or formula defining the bounded variable least square models.
#' Use Y for defining the quantity matrix with values to predict. ´
#' If additional quantity matrices should be used as variables in the models,
#' please refer to these as XPep and/or XProt.
#' All other variables in the formula should refer to columns in \code{annotS}.
#' If BVLS should not be run, set to 'NULL'.
#' Default is defined as 'Y~XPep+XProt'.
#' @param formulaOLS a character string or formula defining the ordinary variable least square models.
#' Use Y for defining the quantity matrix with values to predict. ´
#' If additional quantity matrices should be used as variables in the models,
#' please refer to these as XPep and/or XProt.
#' All other variables in the formula should refer to columns in \code{annotS}.
#' If OLS should not be run, set to 'NULL'.
#' Default is defined as 'Y~Condition'.
#' @param lowBVLS a numeric vector defining lower boundaries of the coefficients of the BVLS models.
#' Elements refer to definition of \code{formulaBVLS}.
#' Default is defined as 'c(-Inf, 0, 0)'
#'@param upBVLS a numeric vector defining upper boundaries of the coefficients of the BVLS models.
...
#' in each BVLS model are added to \code{lowBVLS} and \code{upBVLS}.
#' Added boundaries are automatically set to -Inf for \code{lowBVLS} and Inf for \code{upBVLS}
#' @param returnBVLSmodels a boolean value, set to 'TRUE' for
#' additionally return all BVLS models
#' @param returnOLSmodels a boolean value, set to 'TRUE' for
#' additionally return all OLS models
#' @param LiPonly a boolean value, set to 'TRUE' if you are running the
#' LiPonly version o the package and not providing trypsin-only data
#'
#'@description
...
#'
#' @export
RunModel <- function(spectroList, annotS = NULL, formulaBVLS = "Y~XPep+XProt", formulaOLS = "Y~Condition",
                     lowBVLS = c(-1e9, 0, 0), upBVLS = c(Inf, Inf, Inf), addBVLSbounds = F,
                     returnBVLSmodels = F, returnOLSmodels = F, LiPonly = F){
    ...
    if(!(is.null(formulaBVLS)|is.character(formulaBVLS))&
       (is.null(formulaOLS)|is.character(formulaOLS))){
        stop("Please provide 'formula' in the corrrect class. Use 'character' or 'formula'.")
    }
    ...
    if(length(feat) == 0|length(samples) == 0){
        stop(length(feat), " peptides/proteins in ", length(samples), " samples detected.\nPlease check your input as well as column and row names of all provided data." )
    }
    message("Using ", length(feat), " peptides/proteins and ", length(samples), " samples.")
    ...
    if(!is.null(formulaBVLS)){
        ...
        modelBVLS <- RunBVLS(formulaBVLS, modelMat, lowBVLS, upBVLS, addBVLSbounds)
        resBVLS <- ExtractBVLS(modelBVLS, samples)
        if(is.null(formulaOLS)){
            resALL <- resBVLS
            message("Returning BVLS results including residuals and estimated coefficients.")
        }
...
CreateModelMatrix <- function(spectroList, formula, annotS, samples){
    ...
    modelMat <- lapply(as.list(seq(1, nrow(Y))), function(i){
        ...
        formulaVars <- formula.tools::get.vars(formula)[formula.tools::get.vars(formula)!="Y"]
        e <- environment()
...
RunBVLS <- function(formula, modelMat, lowBVLS, upBVLS, addBVLSbounds){
    ...
    modelRes <- lapply(modelMat, function(data){
        ...
        # if set to true, adding as many as necessary -Inf and Inf to the boundaries of the BVLS
        if(addBVLSbounds){
            nX <- ncol(X)
...
ExtractOLS <- function(OLS, formulaOLS, dfBVLS, coeffPval = "Pr(>|t|)", coeffTval = "t value"){
    ...
    if(is.null(dfBVLS)){
        ...
        message("Estimating p-values while removing degrees of freedom consumed by BVLS.")
        modelPv <- CalcualtePvalAfterBVLS(OLS, coeffTval, dfBVLS)
    }
    if(ncol(modelCoef) == 2){
        formulaOLS <- stats::as.formula(formulaOLS)
        formulaVars <- formula.tools::get.vars(formulaOLS)[formula.tools::get.vars(formulaOLS)!="Y"]
...
# Calculate p-value from OLS model taking degrees of freedom already considered in BVLS into account
CalcualtePvalAfterBVLS <- function(LM,coeffTval, dfBVLS){
    modelPv <- do.call(plyr::rbind.fill.matrix, lapply(LM, function(x){
        ...
        if(df<1){
            warning("Not enough degrees of freedom to estimate p-values, model is not reliable!")
            pv <- stats::setNames(rep(NA, nrow(x$coefficients)), row.names(x$coefficients))
            return(t(pv))
        }
        else{
